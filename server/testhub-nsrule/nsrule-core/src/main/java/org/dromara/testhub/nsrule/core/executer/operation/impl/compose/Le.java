...
public class Le extends Operation {
    ...
    public boolean timeHms(LocalTime t1, LocalTime t2) {
        return !OperationFactory.getOperation(RuleConstant.OperationType.GT).timeHms(t1, t2);
    }

    @Override
    public boolean timeYmdhms(LocalDateTime t1, LocalDateTime t2) {
        return !OperationFactory.getOperation(RuleConstant.OperationType.GT).timeYmdhms(t1, t2);
    }

    @Override
    public boolean timeYmd(LocalDate t1, LocalDate t2) {
        return !OperationFactory.getOperation(RuleConstant.OperationType.GT).timeYmd(t1, t2);
    }

    @Override
    public boolean number(BigDecimal t1, BigDecimal t2) {
        return !OperationFactory.getOperation(RuleConstant.OperationType.GT).number(t1, t2);
    }

    @Override
    public boolean boll(Boolean t1, Boolean t2) {
        return !OperationFactory.getOperation(RuleConstant.OperationType.GT).boll(t1, t2);
    }
