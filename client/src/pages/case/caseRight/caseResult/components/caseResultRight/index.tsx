...
function caseResultRight(props: IProps) {
    ...
    return <>
        <Card className={classnames(styles.verticalTabsCard)}
            bordered={false}
            tabList={operationTabList(executionResult, runXmlParam)}
            onTabChange={onOperationTabChange}
        >
            <Flow flowResult={executionResult.flowResults[tabStatus]} executionResult={executionResult} runXmlParam={runXmlParam} />
        </Card >
    </>
};
            ...
            {props.flowResult.executeResults.map((item, index) => {
                return (
                    <Panel
                        ...
                        ref={componentRef}
                        className={
                            item.runStateItem.result.flag || item.runStateItem.result.bizError
                                ? styles.error
                                : styles.success
                        }
                        extra={
                    ...
                    >
                        <ReactResizeDetector handleWidth onResize={(width, height) => {
                            setWidth(width || 0);
                        }} ></ReactResizeDetector>
                        ...
                        <Collapse defaultActiveKey={["2"]}>
                            ...
                            {(item.runStateItem.runParams != undefined && Object.keys(item.runStateItem.runParams).length > 0) && (
                                <Panel header={i18n('case.text.processlog')} key="2">
                                    <Plugins size={width} flowResult={props.flowResult} executeResult={item} executionResult={props.executionResult} />
                                </Panel>
                            )}
                            {(item.runStateItem.result.content != undefined && Object.keys(item.runStateItem.result.content).length > 0) && (
